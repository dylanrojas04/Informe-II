...
A0 = 1.2;   % Armónico 0 (componente DC)
A1 = 0.56;  % Armónico 1
A2 = -0.45; % Armónico 2
A3 = 0.3;   % Armónico 3
A4 = -0.14; % Armónico 4
...
V_pulse_80 = V0 + V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9;

% Graficar la señal de pulso reconstruida con duty cycle del 80%
figure;
subplot(2,1,1);  % Subgráfico para la señal en el dominio del tiempo
plot(t, V_pulse_80);
title('Señal de Pulso Reconstruida con Duty Cycle 80%');
xlabel('Tiempo (s)');
ylabel('Amplitud');
grid on;

% Aplicar la FFT a la señal
N = length(V_pulse_80);  % Número de puntos en la señal
f_fft = fft(V_pulse_80);  % Calcular la FFT
